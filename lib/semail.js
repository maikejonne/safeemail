...
SEMail.onMessage = function (socket, recvdata, callback) {
    ...
    if (data.cmd === 'user.login') {
        ...
        mymongodb.getUserBaseInfo(dwUID, true, function (err, result) {
            ...
            if(typeof(data.token) === 'string' && typeof(data.lang) === 'string' &&
                data.token.length > 0 && data.token.length <= 128 &&
                (data.platform === 'ios' || data.platform === 'android') ){
                socket.lang = data.lang==='zh'?"zh":"en";
                socket.device_token = data.token;
            ...
            mymongodb.updateUserFieldByUID(dwUID,{
                ...
                mymongodb.removeDeviceTokenFormPushList(socket.device_token);
            });
        });
    }
...
SEMail.onDisConnect = function (socket) {
    ...
    if (socket.bAuth === true){
        mymongodb.removeDeviceTokenFormPushList(socket.device_token,function(){
            mymongodb.addDeviceTokenFormPushList(socket.device_token, socket.strHaseName, socket.lang, socket.platform, function(){
                resetSocketExtData(socket, false);
            });    
        });
    }else{
