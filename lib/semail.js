...
SEMail.onMessage = function (socket, recvdata, callback) {
    ...
    var dwUID = socket.dwUID, retData = {},_pubkey,_hName;
    ...
    if (data.cmd === 'user.login') {
        ...
        mymongodb.getUserBaseInfo(dwUID, true, function (err, result) {
            ...
            socket.strHaseName = Buffer.from(socket.strUserName, 'base64').toString('hex');
        ...
        if (socket.bAuth === false || typeof(data.hash) !== 'string' ||
            typeof(data.to) !== 'string' || typeof(data.signature) !== 'string'||
            data.to.length !== 44 || data.to.length !== 44 || data.to === socket.strUserName ||
        ...
        mymongodb.createNewMailMessage(socket.strHaseName, data.to, data.msglen, data.hash, data.signature, socket.ip, function (err, result) {
            if (err) {
                callback({'message': 'add chatmessage error.'});
            } else {
