...
(function (mymongodb) {
	...
	mymongodb.createNewTemporaryAuth = function(from, content, callback){
	...
	mymongodb.aggregateAdTop = function (cid, bDeleted, nsize, callback) {
		var _query = {
			'_key': {'$regex': "^[topic:]+[0-9]+$"},
			'cid': cid,
			'deleted':0
		...
		db.client.collection('objects').aggregate(pipeline).toArray(function (err, result) {
			if (err) {
				return callback(err);
			}
			callback(null, result);
	...
	mymongodb.getMailSession = function(uid, callback) {		
		db.client.collection('mailsession').findOne({
			'_key':'sid:' + uid,
			'expire':{'$gt': Date.now()}
		},
		{
	...
	mymongodb.deleteExpireMailSession = function(callback){
		db.client.collection('mailsession').deleteMany({
			'_key': {'$regex': "^[sid:]+[0-9]+$"},
			'expire':{'$lt': Date.now()}
		},
		function(){
	...
	mymongodb.deleteAllMailSession = function(){
		db.client.collection('mailsession').deleteMany({});
	};

	mymongodb.deleteMailSession = function(uid, callback) {
		db.client.collection('mailsession').deleteOne({
			'_key':'sid:' + uid
		},
		function(){
			callback(null);
	...
	mymongodb.createNewMailSession = function(randkey, callback){
		var sessionData = {
			rkey:randkey,
		...
		async.waterfall([
			function (next) {
				db.incrObjectField('global', 'msess_nextUid', next);
			},
			function (uid, next) {
				sessionData.uid = uid;

				db.client.collection('mailsession').updateOne(
					{
						_key: 'sid:' + uid
					},
					{
