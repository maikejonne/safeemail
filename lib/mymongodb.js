...
(function (mymongodb) {
	...
	mymongodb.queryHashHeadInfo = function(pubkey, hash, nlen, callback){
		callback = callback || function() {};
		db.client.collection('enchash').findOne({
				'_key':"head:" + pubkey + ":"+nlen+":"+hash
			},
			{
				'projection': {
					'_id': 0,
	...
	mymongodb.queryHashAuthor = function(pubkey, name, callback){
		callback = callback || function() {};
		db.client.collection('enchash').findOne({
				'_key':'author:' + pubkey + ":"+name
			},
			{
				'projection': {
					'_id': 0,
