...
router.post('/downloadattachment', function (req, res) {
    ...
    var hName = _objpubkey.toString('hex');
    mymongodb.getMailMessageAttachmentInfo(hName, req.body.addr, req.body.mid, function(err, result){
        if (err || result === null) {
            return res.sendStatus(400);
        }
        var _attachmenturl = nconf.get('url') + "/assets/uploads/files/mails/" + req.body.mid + "_" + result.createdate + ".dat";
...
router.post('/queryattachmentpasswd', function (req, res) {
    ...
    var hName = _objpubkey.toString('hex');
    mymongodb.getMailMessageAttachmentPassword(hName, req.body.addr, req.body.mid, req.body.filehash, req.body.filelen, function(err, result){
        if (err || result === null) {
            return res.sendStatus(400);
        }
        res.send({"hash":result.uphash,"nlen":result.upcurlen,"passwd":result.passwd,'signature':result.signup});
        var _ipaddress = req.headers['x-forwarded-for'] || req.connection.remoteAddress;
        mymongodb.deleteMailMessage(hName, req.body.addr, req.body.mid, req.body.signature, _ipaddress);
        
        fs.unlinkSync(nconf.get('upload_path') + "/files/mails/" + req.body.mid + "_" + result.createdate + ".dat");
    });
});
...
router.post('/uploadattachment', multipartMiddleware, function (req, res) {
    ...
    var strpath = nconf.get('base_dir') + "mailattchment/";
    //dest = fs.createWriteStream(output);

    mymongodb.getMailMessageAttachmentAuth(_hName, mid, function (err, result) {
        ...
        if (nSize > result.upmsglen) {
            ...
            source.on('end', function () {
                ...
                mymongodb.updateMailMessageAttachment(_hName, mid, nSize, 0, 0, "", 0, function (err,updateres) {
                    if (err) {
                        fs.unlinkSync(distFilename);
                        return res.sendStatus(400);
                    }
                        ...
                        doDealAttachment(distFilename,
                            ...
                            function(err,result){
                                if(err){
                                    ...
                                    mymongodb.updateMailMessageAttachment(_hName, mid, nSize, 1, result.encrypt, result.dlhash, result.dllen, function (err,updateres) {
                                        if (err) {
                                            fs.unlinkSync(distFilename);
                                            return res.sendStatus(400);
                                        }
...
router.post('/abouttop', function (req, res) {
    if (!req.body) {
        return res.sendStatus(400);
    }

    var retData = {}, i;
    mymongodb.queryHeadlinesArticles(6, 0, false, false, function (err, ret) {
        if (err) {
            return res.send(JSON.stringify({'errmsg': 'refresh data error.'}));
        }
        mymongodb.queryPostFromFourmArticles(ret, 0, function (err) {
            ...
            if (ret.length < 10) {
                retData.ts = -1;//表示不再接受查询了
            } else {
                retData.ts = ret[ret.length - 1].timestamp;
            }
...
router.post('/moreabouttop', function (req, res) {
    if (!req.body) {
        return res.sendStatus(400);
    }

    var retData = {}, i;
    mymongodb.queryHeadlinesArticles(7, 0, false, true, function (err, ret) {
        if (err) {
            return res.send(JSON.stringify({'errmsg': 'refresh data error.'}));
        }
        mymongodb.queryPostFromFourmArticles(ret, 0, function (err) {
...
router.post('/aboutgeneral', function (req, res) {
    if (!req.body || req.body.timestamp === null || !utils.isNumber(req.body.timestamp, 10) || req.body.timestamp < 0) {
        return res.sendStatus(400);
    }

    var retData = {}, i;

    mymongodb.queryHeadlinesArticles(8, req.body.timestamp, false, false, function (err, result) {
        if (err) {
            return res.send(JSON.stringify({'errmsg': 'refresh data error.'}));
        }
...
router.post('/subarticle', function (req, res) {
    ...
    mymongodb.queryArticleByToPIDAndTID(req.body.topid, req.body.tid, function (err, result) {
        if (err) {
            res.send(JSON.stringify({'errmsg': 'refresh data error.'}));
        } else {
            if (result.length > 0) {
                if (req.body.type === 1) {
                ...
                res.send(JSON.stringify({errmsg: 'article not exist.'}));
            }
        }
    });
});
