...
/*
* @api {post} /getmoreunreads 06.Get Unread Message
* @apiName getunreads
* @apiGroup SeMail
* @apiDescription 用户在登录/非登录状态下，获取属于自己尚未阅读的消息。
*
* @apiParam {String} username 用户的公钥。
* @apiParam {String} signature 用户对Base64.encode(session-randkey)字符串的签名。
* @apiParam {String} addrs 获取某指定公钥组发送的消息。
* @apiSuccessExample {json} Success-Response:
*     HTTP/1.1 200 OK
*     {
*       "addrs": [],
...
*  @apiError 400 Bad Request 
*/
router.post('/getmoreunreads', function (req, res) {
    if (!req.body || !req.session || typeof(req.session.randkey) !== "string" || req.session.randkey.length !== 32 ||
        typeof(req.body.username) !== 'string' || req.body.username.length !== 44 ||
        typeof(req.body.signature) !== 'string' ||
        req.body.addrs === null || !isArray(req.body.addrs) || req.body.addrs.length === 0 || req.body.addrs.length > 50) {
        return res.sendStatus(400);
    }
    ...
    for (var i = 0; i < req.body.addrs; i++) {
        if(typeof(req.body.addrs[i]) !== 'string' || req.body.addrs[i].length !== 44){
            return res.sendStatus(400);
        }
    }

    mymongodb.getFriendsUnreads(hName, req.body.addrs, function(err, results){
        if(err){
            return res.sendStatus(400);
        }
